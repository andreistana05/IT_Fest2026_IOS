...
FallDetector is an iOS app that receives instant push notification alerts when a fall is detected on a monitored person's device, and lets the user send an emergency SOS with their GPS location to all their contacts.
...
Fall detected on monitored device
   ...
   Node.js server  (./server)
        ...
        ▼
   FallDetector iOS app  ←─ push notification + Firestore live listener
```

1. The person at risk has FallDetector installed and their phone number registered in the system.
...
### Alert monitoring
- Receive instant push notifications when someone you monitor has a fall detected.
...
- The app fetches your saved emergency contacts from Firestore, composes a message with your current GPS coordinates (Apple Maps + Google Maps links), and sends individual SMS messages to each contact using the native Messages app.
...
- Location is also saved to your Firestore profile.

### User profile
- View and edit your display name, phone number, and profile photo.
- Your phone number is what other users enter when they want to monitor you.
...
|---|---|---|
| POST | `/alert` | Receive a fall event and fan out to all followers |
...
- A physical iOS device (push notifications require a real device)
...
1. **Create an account** — register with your email, password, and phone number. Your phone number is how the system identifies you as someone to alert.
2. **Add contacts to monitor** — go to **Monitored contacts** and enter the phone numbers of people you want to watch over. Their number must match what is registered on their device (include the country code, e.g. `+40712345678`).
...
5. **Send SOS** — if you have fallen yourself, tap **I fell — alert my contacts**. Your GPS location will be sent by individual SMS to each of your saved contacts instantly.
